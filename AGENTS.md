...
### Coding Challenge Verification Pipeline
When a coding challenge (either via `coding-challenge` or `topic-outline` queue items) is generated by Gemini:
1. Validate JSON schema via Zod (`codingChallengeSchema`), ensuring existence of `title`, formats, `starterCode`, and `referenceSolution`.
2. Reject malformed cases or duplicate test case inputs. Ensure at least one case is marked hidden.
3. Securely execute the reference solution against all generated test inputs using the Piston executor client.
4. Validate that the reference solution does not crash (non-zero exits or stack traces) on any test case.
5. Verify that the output computed by the reference solution exactly matches the AI-generated `expectedOutput` (normalized via `compareOutputs`).
6. If any step fails, the queue item is failed and scheduled for clean regeneration up to 3 times, after which it permanently fails and the challenge status is marked `failed`. Only fully validated challenges are set to `status: "ready"`.
